...
| A91 | Изменять IAM и billing может только административная identity, отделённая от runtime/deploy | Компрометация приложения или CD снимает последний независимый cost control | ✅ read-only IAM-сверка 2026-09-11: единственный Project Owner — `zhirikhin.kirill@gmail.com`; у `github-deployer` и `insights-api-runtime` административных ролей нет | M | 2026-09-11 |
| A92 | Деплойная identity не может назначить более сильную runtime identity или снять Spend Cap | Вредоносный workflow обходит ограничения текущего service account и финансовый предел | ✅ повторно сверено 2026-09-11: `github-deployer` имеет `artifactregistry.writer`, `run.developer` и `serviceAccountUser` только на `insights-api-runtime`; GitHub WIF ограничен repository ID `1199475908` и `main`. У runtime единственная project-role `vertexAiGeminiInvoker` с одним permission `aiplatform.endpoints.predict`; административных прав и user-managed ключей у runtime/deploy нет | M | 2026-09-11 |
| A93 | После миграции Vertex AI остаётся единственным доступным AI-каналом приложения | Сохранённая Anthropic federation позволяла бы тратить вне Vertex Spend Cap | ✅ владелец сообщил об архивировании правила `fdrl_01TMxGCrboyf8U4HeN94msmS` 2026-09-18; Anthropic API/Console независимо агентом не прочитаны. Новая production-ревизия работает без четырёх `ANTHROPIC_*` и прошла smoke; в старых ревизиях ещё может быть Anthropic-код, но новые токены через архивированное правило не выдаются. Остаток: ранее выданные токены до 10 минут и неинвентаризированные иные правила/ключи организации не исключены | M | 2026-09-18 |
...
| 8. Production rollout | выполнен 2026-09-18; ожидает подтверждения закрытия пользователем | PR #187 и #64 слиты, CI и деплой зелёные; новая ревизия обслуживает 100% трафика, production smoke дал ответ. Anthropic-ревизия `insights-api-00057-rlj`, сохранявшаяся для rollback, после архивирования правила стала бесполезной и удалена на этапе 9; точка отката теперь `insights-api-00058-kdq` — тот же Vertex-код до очистки env |
| 9. Удаление Anthropic | выполняется с 2026-09-18 | новая ревизия работает без четырёх `ANTHROPIC_*`; владелец подтвердил архивирование внешнего правила; неверное уведомление о получателе данных исправлено, закреплено A99 и выложено в production (PR #189, deployment `dpl_7gnnAh4bUoPbYiDk5JYKo2xe9oW9`); 32 ревизии эпохи Anthropic удалены, чем исправлено и обоснование A24. Остаются визуальная проверка строки владельцем и отзыв взятых на задачу доступов |
| 10. Итоговая модель угроз | не начат | production DFD отражён после live-проверки; окончательное закрытие старого канала и пересмотр остатков — после этапа 9 |
