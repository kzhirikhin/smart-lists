...
| TB5-legacy | FastAPI → Anthropic | закрыт 2026-09-18: кода нет, `ANTHROPIC_*` удалены из ревизии, внешнее правило federation архивировано владельцем. Прежняя ревизия `insights-api-00057-rlj` ещё существует, но получить токен по архивированному правилу не может |
...
| A25 | Код внутри контейнера доверен | RCE получает ADC runtime identity и может вызывать Vertex AI в пределах `aiplatform.endpoints.predict`; прямой защиты от исполнения кода нет | ❌ ограничение blast radius, не устранение RCE: runtime не имеет IAM/Billing/Budget Admin, Vertex Spend Cap независим. Anthropic federation rule, ранее дававшее второй AI-канал той же identity, архивировано владельцем 2026-09-18; уже выданные Anthropic tokens могли оставаться действительными до 10 минут после архивирования. Старые ревизии могут содержать Anthropic-код, но после истечения токенов не могут получить новый доступ через это правило | — | 2026-09-18 |
...
| A99 | Уведомление в панели AI называет фактического получателя пользовательских данных на всех поддерживаемых локалях | При смене провайдера пользователь видит ложное описание передачи содержимого списка и заметок других участников | ✅ регрессия обнаружена 2026-09-18: после production rollout строка всё ещё называла Anthropic. Исправлено на Vertex AI (Google Cloud) в `ru/vi/en/ja`. Полнота берётся из источника факта: `test/messages.test.ts` перечисляет локали по `routing.locales` и отдельным тестом сверяет этот набор с каталогом `messages/`, поэтому пятый язык не может выпасть из проверки молча; оба режима поломки проверены мутацией 2026-09-18. E2E требует от видимой строки панели текст «Vertex AI». Остаток: до выкладки этой ветки production показывает прежнее имя провайдера, поэтому допущение держится в репозитории, но ещё не у пользователя | T | каждый CI |
...
| 8. Production rollout | выполнен 2026-09-18; ожидает подтверждения закрытия пользователем | PR #187 и #64 слиты, CI и деплой зелёные; новая ревизия обслуживает 100% трафика, production smoke дал ответ. Rollback-ревизия `insights-api-00057-rlj` сохранена, но после архивирования правила на этапе 9 откат на неё уже не вернул бы работающий AI-канал |
| 9. Удаление Anthropic | выполняется с 2026-09-18 | новая ревизия работает без четырёх `ANTHROPIC_*`; владелец подтвердил архивирование внешнего правила; неверное уведомление о получателе данных исправлено и закреплено A99. Остаются production-выкладка исправленного уведомления, решение по rollback-ревизии `insights-api-00057-rlj` и отзыв взятых на задачу доступов |
