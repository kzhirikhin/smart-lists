...
обработчику, даже несмотря на то, что обе стороны размещены в GCP. До
production-переключения основной DFD выше намеренно не перерисовывается.
...
а `serviceAccountUser` выдан ему только на `insights-api-runtime`. После выдачи
runtime права вызова Vertex AI этот снимок требуется переснять: сейчас такого
права ещё нет и вызов Gemini из production невозможен.
...
| A80 | Деплойная identity не может назначить более сильную runtime identity или снять Spend Cap | Вредоносный workflow обходит ограничения текущего service account и финансовый предел | ✅ `github-deployer` имеет `artifactregistry.writer`, `run.developer` и `serviceAccountUser` только на `insights-api-runtime`; GitHub WIF ограничен repository ID `1199475908` и `main`. Это внешнее состояние и требует повторной сверки после IAM-изменений Vertex | M | 2026-09-04 |
...
| A82 | Gemini вызывается через ADC runtime identity без API key | Ключ возвращает долгоживущий переносимый секрет и отдельный путь аутентификации | ❌ **ещё не реализовано:** у `insights-api-runtime` нет права Vertex AI, Google SDK не установлен, текущий код использует Anthropic workload identity federation | — | — |
...
внешние IAM и billing controls могут дрейфовать без сигнала между ручными
сверками.
...
| 2. Security-контракты | не начат | тесты должны упасть до реализации Vertex-клиента и закрыть auth, endpoint, capabilities, logging и outbound surface |
| 3. Vertex IAM | не начат | keyless ADC и least privilege runtime без административных полномочий |
| 4. Сравнение Flash-Lite | не начат | выбор модели прямым сравнением кандидатов без оглядки на Haiku |
...
| 10. Итоговая модель угроз | не начат | целевой DFD становится фактическим только после live-проверки |
