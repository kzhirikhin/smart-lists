...
**Последнее обновление:** 2026-09-18 (миграция на Vertex AI завершена, этапы 1–10 закрыты)
...
новые токены по этому правилу не выдаются. Ревизии эпохи Anthropic удалены на
...
не вернул бы работающий AI-канал. Живут две ревизии — `insights-api-00059-fhh`
под трафиком и `insights-api-00058-kdq` как точка отката. Уведомление о передаче
...
введено, остался только access-log ревизии. Второе: Spend Caps подтверждаются
лишь консольным скриншотом, потому что `billingbudgets.googleapis.com` в проекте
не включён и CLI их не видит; включение этого API — самый дешёвый способ убрать
слепое пятно.
